...
function calculateNumerology() {
  ...
  const soulUrgeNumber = reduceToSingleDigit(
    getNumerologyNumber(name, isVowel)
  );
  const personalityNumber = reduceToSingleDigit(
    getNumerologyNumber(name, isConsonant)
  );

  const letterChaldeanMap = getLetterChaldeanMapping(name);
  ...
  const destinyText = getDestinyNumberInterpretation(destinyNumber);
  const soulText = getSoulNumberInterpretation(soulUrgeNumber);
  const dreamText = getDreamNumberInterpretation(personalityNumber);

  const calculateChaldeanSum = calculateChaldeanNumberSum(name);
  const resultDiv = document.getElementById("result");
  resultDiv.innerHTML = `
  <p class="name" ><span id="span">Chaldean Name Numerology Reading Of :        </span>${letterChaldeanMap}</p>
  

  <p class="text-interpretation">Compound Name Number/Namank (Numerology Total of Your Name): <span id="number-design"> ${calculateChaldeanSum}</span>

    <p class="text-interpretation"><span id="span">Destiny Number:</span> The Expression number, which describes who you are, and what you are, or what you become. </br></br>
    <span id="span">Name Destiny/Expression Number or Namanak : </span> <span id="number-design"> ${destinyNumber}</span> <br/>
   ${destinyText}
    </p>

    <p class="text-interpretation"><span id="span">Soul Urge Number: </span>The Heart Desire number, which describes your inner potentials and inner resources.</br></br>
    <span id="span">Soul Urge/Heart Desire Number: </span> </span> <span id="number-design"> ${soulUrgeNumber} </span> <br/>
    ${soulText}</p>

    <p class="text-interpretation"><span id="span">Dream Number:</span> The Personality number which describes outer personality, indeed your first impression on others.</br></br>
    <span id="span">Name Dream/Personality Number:</span> </span> <span id="number-design"> ${personalityNumber}</span>  <br/>
    ${dreamText}</p>.
  `;
}
